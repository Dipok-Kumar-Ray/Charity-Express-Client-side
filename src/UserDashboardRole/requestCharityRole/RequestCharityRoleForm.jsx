...
const RequestCharityRoleForm = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      axios
        .get(`https://charity-ex-server-side.vercel.app/charity-requests/${user.email}`)
        .then((res) => {
          if (
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post("https://charity-ex-server-side.vercel.app/create-payment-intent", {
        amount,
      });
      ...
      if (result.paymentIntent.status === "succeeded") {
        ...
        await axios.post("https://charity-ex-server-side.vercel.app/charity-requests", {
          name: user.displayName,
          email: user.email,
          organization,
          mission,
